...
export class VendorComparisonComponent implements OnInit {
    ...
    onVendorAttachmentSelect(event: Event, vendorId: number): void {
        const input = event.target as HTMLInputElement;
        if (!input.files?.length) return;
        this.vendorAttachmentFiles[vendorId] = input.files[0];
        this.vendorAttachmentNames[vendorId] = input.files[0].name;
    }
    ...
    get rankedVendors(): { vendor: VendorRef; rank: number; total: number }[] {
        return this.selectedVendors.map((v) => ({ vendor: v, rank: this.getVendorRank(v.vendor_id), total: this.getVendorTotal(v.vendor_id) })).sort((a, b) => a.rank - b.rank);
    }
